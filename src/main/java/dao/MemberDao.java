...
public class MemberDao {
    ...
    public static Member getByEmailIdAndPassword(String emailId, String password) {
        try {
            ...
            if(getAll(stmt).isEmpty())
                return null;
            Member m = getAll(stmt).get(0);
            return m;
        } catch (SQLException e) {
            logger.error(e.getMessage(), e);
            return null;
    ...
    public static List<Member> getAllEmail(String emailId) {
        List<Member> list = new ArrayList<>();
        try {

            PreparedStatement stmt = conn.prepareStatement("select * from member where email like ?");
            stmt.setString(1, "%" + emailId + "%");
