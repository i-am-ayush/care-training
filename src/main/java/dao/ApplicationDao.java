...
public class ApplicationDao {
    ...
    public static List<Job> getNotAppliedJobsList(int userId) {
        List<Job> resultList = new LinkedList<>();
        try (PreparedStatement stmt = conn.prepareStatement("SELECT * FROM job WHERE id NOT IN " +
                "(SELECT jobId FROM Application WHERE memberId = ?) AND status='ACTIVE'")) {
            stmt.setInt(1, userId);
            try (ResultSet res = stmt.executeQuery()) {
                while (res.next()) {
    ...
    public static boolean deleteByApplicationId(int appId) {
