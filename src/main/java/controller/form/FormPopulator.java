...
//            }
    ...
    public static <T extends Form> T populateFormFromDatabase(HttpServletRequest request, Class<T> tClass) throws IllegalAccessException, InstantiationException, NoSuchMethodException, InvocationTargetException {
        Field[] variables = tClass.getDeclaredFields();
        T t = tClass.newInstance();
        for (Field f : variables) {
            System.out.println(f.getType());
            ...
            if(f.getType()==int.class) {
                System.out.println("int1");
                int i=Integer.parseInt((String) request.getAttribute(f.getName()));
                fieldSetter.invoke(t, i);
                System.out.println("int");
            }
            else if(f.getType()==double.class) {
                System.out.println("int2");
                fieldSetter.invoke(t, (Double)(request.getAttribute(f.getName())));
                System.out.println("double");
            }
//            else if(f.getType()==Enum.class) {
//
